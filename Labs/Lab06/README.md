...
Причина следующая коммутатор S3 подключен к корнемому коммутатору S2 через интерфейс F0/4, коммутатор S1 подключен к корнемому коммутатору S2 через интерфейс F0/2. Из-за того, что номер порта через который подключен коммутатор S3 к S2 больше, был выбран вышеуказанный порт в качестве альтернативного и заблокирован.**   
### Часть 3: Наблюдение за процессом выбора протоколом STP порта, исходя из стоимости портов.
...
**~У меня данные настройки не применились~**  
